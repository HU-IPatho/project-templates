...
★DTU/DTE は各群に複製が必要（各群 n>=2）。取得の前に config.samples の群別複製数を検査し、
  n=1 の群があれば fail-fast で停止する（下記 check_replicates）。
"""
...
def check_replicates(cfg: dict) -> None:
    """各群の複製数を検査し、n=1 の群があれば fail-fast（DTU/DTE は複製必須）。

    量の差だけを見たい n=1 実験は rnaseq-secondary（② edgeR n=1 HK 分散）へ誘導する。
    """
    gcol = (cfg.get("design", {}) or {}).get("group_col", "group")
    counts: dict[str, int] = {}
    ...
    singletons = {g: n for g, n in counts.items() if n < 2}
    if singletons:
        detail = ", ".join(f"{g}(n={n})" for g, n in singletons.items())
        raise SystemExit(
            "DTU/DTE は各群に複製が必要（各群 n>=2）。n=1 の群: " + detail + "。"
...
def main(dry_run: bool = False) -> None:
    cfg = load_config()
    check_replicates(cfg)           # 複製ガード（n=1 は即停止 → ② へ誘導）
