...
取得の前に config.samples の群別複製数を検査し、n=1 の群があれば fail-fast する
（DTU/DTE は各群 n>=2 が必須。量差だけの n=1 は rnaseq-secondary へ誘導）。

実行（プロジェクトルートから）:
    python3 analysis/01_fetch.py            # 複製検査 → 取得実行
    python3 analysis/01_fetch.py --dry-run  # 複製検査 → 取得計画だけ表示
