...
#   - config.salmon.num_bootstraps > 0 のとき --numBootstraps を付与する（swish=fishpond の
#     inferential replicates 用。既定 0＝軽い salmon。DRIMSeq/DEXSeq/DESeq2/edgeR は bootstrap 不要）。
...
NBOOT="$(CFG get salmon.num_bootstraps)"  # inferential replicates 数（swish 用・既定 0）
...
# num_bootstraps > 0 のときだけ --numBootstraps を足す（0/非数値なら無効化）。
BOOT=()
if [ "${NBOOT:-0}" -gt 0 ] 2>/dev/null; then
  BOOT=(--numBootstraps "$NBOOT")
  echo "[salmon] inferential replicates: --numBootstraps $NBOOT（swish 用）"
fi
